...
INFRA_DIR="./infrastructure/kubernetes/production"
DB_NAME="meeting_db"

echo -e "${BLUE}====================================================${NC}"
echo -e "${BLUE}   Meeting Automation - Production k3s Deployment      ${NC}"
echo -e "${BLUE}====================================================${NC}"
...
if [ -f "./kubeconfig-production.txt" ]; then
    export KUBECONFIG="$(pwd)/kubeconfig-production.txt"
elif [ -f "$HOME/.kube/config-production" ]; then
    export KUBECONFIG="$HOME/.kube/config-production"
fi

if ! kubectl config get-contexts 2>/dev/null | grep -q "production-cluster"; then
    echo -e "${RED}Error: Context 'production-cluster' not found.${NC}"
    exit 1
fi
kubectl config use-context production-cluster > /dev/null
echo -e "${GREEN}Context: production-cluster${NC}"
...
echo -e "  Warte auf CNPG meeting-db (HA, 2 Instances)..."
...
kubectl create secret generic e2e-test-user \
    --namespace "${NAMESPACE}" \
    --from-literal=E2E_TEST_USER_EMAIL="e2e-tester@meeting-automation.com" \
...
LOGIN_RESP=$(curl -sk https://meeting-automation.com/api/v1/auth/login \
...
EXTERNAL=$(curl -sf -o /dev/null -w '%{http_code}' https://meeting-automation.com 2>/dev/null || echo "000")
if [ "$EXTERNAL" = "200" ]; then
    echo -e "${GREEN}  ✅ meeting-automation.com 200${NC}"
else
    echo -e "${RED}  ❌ meeting-automation.com ${EXTERNAL}${NC}"
fi
...
echo -e "${BLUE}====================================================${NC}"
echo -e "  Frontend:    https://meeting-automation.com"
echo -e "  Backend:     https://meeting-automation.com/api/v1/..."
echo -e "  n8n:         kubectl port-forward svc/n8n-staging 5678:5678 -n ${NAMESPACE}"
echo -e "  LiveKit:     ws://meeting-automation.com"
echo -e ""
