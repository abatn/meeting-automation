    ...
    <ErrorBoundary>
      <Box sx={{ minHeight: '100vh' }}>
        ...
        <Routes>
          ...
          <Route 
            path="/login" 
            element={authState === 'unauthenticated' ? <LoginForm /> : <Navigate to="/" replace />} 
          />
          ...
          <Route 
            ...
            element={
              authState === 'authenticated' && user ? (
                <MainLayout>
                  <Box sx={{ p: 3 }}>
                    <MFASetup qrCodeUrl="https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=ExampleSecret" secret="JBSWY3DPEHPK3PXP" />
                  </Box>
                </MainLayout>
              ) : (
                <Navigate to="/login" replace />
