...
export const meetingsApi = {
  ...
  uploadRecording: async (meetingId: string, file: File, recordingId?: string) => {
  ...
  uploadStreamChunk: async (uploadId: string, fileKey: string, partNumber: number, blob: Blob) => {
    const formData = new FormData();
  ...
  stopStream: async (recordingId: string, uploadId: string, fileKey: string, parts: any[]) => {
    const response = await api.post(`/recordings/stream/stop/${recordingId}?upload_id=${uploadId}&file_key=${fileKey}`, { parts });
    return response.data;
  },
