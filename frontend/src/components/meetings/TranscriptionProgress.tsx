...
import { Box, LinearProgress, Typography, CircularProgress, useTheme } from '@mui/material';
...
const TranscriptionProgress: React.FC<Props> = ({ recordingId, onComplete }) => {
    ...
    <Box sx={{ width: '100%', mt: 2, mb: 2, direction: isRtl ? 'rtl' : 'ltr' }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center' }}>
        ...
        {!isConnected && !error && progress < 100 && (
          <CircularProgress size={16} sx={{ ml: isRtl ? 0 : 1, mr: isRtl ? 1 : 0 }} />
        )}
      </Box>
    </Box>
  );
