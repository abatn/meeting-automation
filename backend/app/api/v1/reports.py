...
from app.models.meeting import Meeting as MeetingModel, MeetingStatus, Participant as ParticipantModel
from app.models.action import Action as ActionModel, ActionStatus, Assignment as AssignmentModel
...
async def get_dashboard_data(
    ...
    if role == 'dg':
        ...
        completed_meetings_query = select(func.count(MeetingModel.id)).where(MeetingModel.status == MeetingStatus.COMPLETED)
        ...
        # 2. Pending Actions
        pending_actions_query = select(func.count(ActionModel.id)).where(ActionModel.status == ActionStatus.PENDING)
        ...
        action_dist_query = select(ActionModel.status, func.count(ActionModel.id)).group_by(ActionModel.status)
        action_dist_res = await db.execute(action_dist_query)
        action_status_distribution = {status.name.lower(): count for status, count in action_dist_res.all()}
        ...
        return {
            ...
            "action_status_distribution": {
                ...
                "open": action_status_distribution.get('pending', 0) # Mapping 'pending' from DB to 'open' for frontend
            }
        }
        ...
        if not managed_user_ids:
            return ManagerDashboard(meeting_stats=MeetingStats(total=0, completed=0, scheduled=0), action_stats=ActionStats(pending=0, completed=0), team_members_count=0, team_productivity=[], efficiency_trend=[])
        ...
        completed_team_meetings = (await db.execute(team_completed_meetings_query)).scalar_one()
        ...
        completed_team_actions = (await db.execute(team_completed_actions_query)).scalar_one()

        return ManagerDashboard( \
            meeting_stats=MeetingStats(total=total_team_meetings, completed=completed_team_meetings, scheduled=total_team_meetings - completed_team_meetings), # Annahme: Geplant = Total - Abgeschlossen
            action_stats=ActionStats(pending=pending_team_actions, completed=completed_team_actions),
            team_members_count=len(managed_user_ids), # Anzahl der direkt verwalteten Benutzer
            team_productivity=[],
            efficiency_trend=[]
        )
        ...
        my_upcoming_meetings_query = select(func.count(MeetingModel.id)) \
            ...
            .where(MeetingModel.end_time > datetime.utcnow()) # Nur zukünftige Meetings
            
        my_upcoming_meetings = (await db.execute(my_upcoming_meetings_query)).scalar_one()
        ...
        return {"my_upcoming_meetings": my_upcoming_meetings, "my_open_actions": my_open_actions}
