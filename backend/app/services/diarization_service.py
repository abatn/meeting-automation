...
class DiarizationService:
    ...
    def get_pipeline(cls):
        """
        Singleton initialization of the pyannote pipeline.
        """
        if not PYANNOTE_AVAILABLE:
            logger.warning(
                "pyannote.audio is not installed. Diarization will be disabled."
            )
            return None
        ...
        if cls._pipeline is None:
            ...
            if not token:
                logger.error(
                    "HUGGINGFACE_TOKEN is missing. Cannot initialize diarization."
                )
                return None
    ...
    def resample_audio(cls, input_path: str, output_path: str) -> bool:
        """
        Resamples audio to 16kHz mono using FFmpeg.
        """
        try:
    ...
    async def diarize(cls, audio_file_path: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if not cls.resample_audio(input_path, resampled_path):
                logger.warning("Resampling failed, using original audio.")
