...
class ReportService:
    def __init__(self, db: AsyncSession):
        self.db = db
        self.redis_client = redis.Redis(host=settings.REDIS_HOST, port=settings.REDIS_PORT, db=1)

    async def _get_cached_or_compute(self, key: str, compute_func, ttl: int = 1800) -> Any:
        """Helper: Versucht Daten aus Redis zu laden, berechnet sonst neu und speichert sie"""
        try:
    ...
    async def get_meeting_stats(self, period: str = "month") -> dict:
        ...
        async def compute():
            date_filter = datetime.utcnow() - timedelta(days=30 if period == "month" else 365)
            query = select(Meeting.status, func.count(Meeting.id)).where(Meeting.start_time >= date_filter).group_by(Meeting.status)
    ...
    async def get_action_completion_rate(self, days: int = 30) -> dict:
        ...
        async def compute():
            ...
            now = datetime.utcnow()
            query = select(
                func.sum(case((Action.status == 'completed', 1), else_=0)).label('completed'),
                func.sum(case((Action.status == 'pending', case((Action.due_date < now, 1), else_=0)), else_=0)).label('overdue'),
                func.sum(case((Action.status == 'pending', case((Action.due_date >= now, 1), else_=0)), else_=0)).label('pending')
            )
            result = await self.db.execute(query)
            row = result.first()
            return {
    ...
    async def get_efficiency_trend(self, months: int = 6) -> List[dict]:
        ...
        async def compute():
            ...
            # in Produktion: Komplexere aggregierte Zeitabfrage nötig
            trend = []
            now = datetime.utcnow()
            ...
            return trend
        return await self._get_cached_or_compute(f"reports_efficiency_{months}", compute, ttl=86400)
