...
class TestPhase7MinIOIntegration:
    ...
    @pytest.mark.asyncio
    async def test_upload_recording_creates_client_id_prefix(self, db: AsyncSession, authenticated_user_a):
        """
        ...
        """
        # Setup
        current_user = authenticated_user_a
        ...
        assert recording.file_path.startswith(current_user.client_id)
    ...
    @pytest.mark.asyncio
    async def test_stream_recording_creates_client_id_prefix(self, db: AsyncSession, authenticated_user_a):
        """
        ...
        # Assert: file_key contains client_id prefix
        assert result["file_key"].startswith(current_user.client_id)
    ...
    @pytest.mark.asyncio
    async def test_presigned_upload_url_generation(self, db: AsyncSession, authenticated_user_a):
        """
        ...
        service = RecordingService(db)
        file_key = f"{current_user.client_id}/recordings/{meeting.id}/{uuid.uuid4()}_test.wav"
    ...
    @pytest.mark.asyncio
    async def test_presigned_download_url_generation(self, db: AsyncSession, authenticated_user_a):
        """
    ...
    @pytest.mark.asyncio
    async def test_api_presigned_upload_url_endpoint(self, db: AsyncSession, authenticated_user_a):
        """
        ...
        assert data["file_key"].startswith(current_user.client_id)
        assert "X-Amz-Signature" in data["presigned_url"]
        print(f"✅ API presigned upload endpoint works: {data['file_key']}")

    @pytest.mark.asyncio
    async def test_api_presigned_download_url_endpoint(self, db: AsyncSession, authenticated_user_a):
        """
    ...
    @pytest.mark.asyncio
    async def test_cross_tenant_isolation_upload_url(self, db: AsyncSession, authenticated_user_a, authenticated_user_b):
        """
    ...
    @pytest.mark.asyncio
    async def test_recording_not_found_returns_404(self, db: AsyncSession, authenticated_user_a):
        """
    ...
    @pytest.mark.asyncio
    async def test_onlyoffice_config_uses_public_url(self, db: AsyncSession, authenticated_user_a):
        """
    ...
    @pytest.mark.asyncio
    async def test_recording_file_key_format_validation(self, db: AsyncSession, authenticated_user_a):
        """
        ...
        expected_prefix = f"{current_user.client_id}/recordings/{meeting.id}/"
        ...
        assert parts[0] == current_user.client_id
    ...
    @pytest.mark.asyncio
    async def test_presigned_url_expiry_configuration(self, db: AsyncSession, authenticated_user_a):
        """
    ...
    @pytest.mark.asyncio
    async def test_audit_logging_for_presigned_url_generation(self, db: AsyncSession, authenticated_user_a):
        """
    ...
    @pytest.mark.asyncio
    async def test_meeting_client_id_isolation_in_presigned_endpoint(self, db: AsyncSession, authenticated_user_a, authenticated_user_b):
        """
...
class TestPhase7MinioBucketPolicy:
    ...
    @pytest.mark.asyncio
    async def test_client_id_prefix_prevents_path_traversal(self, db: AsyncSession, authenticated_user_a):
        """
        ...
        legitimate_file_key = f"{current_user.client_id}/recordings/{meeting.id}/{uuid.uuid4()}_test.wav"
        
        # Verify legitimate key is properly formed
        assert not ".." in legitimate_file_key
        assert legitimate_file_key.startswith(current_user.client_id)
...
@pytest.fixture
async def authenticated_user_b(db: AsyncSession):
