...
4.  **Redundanz-Management (Update 15.03.2026)**:
    *   Sicherstellung der Konsistenz zwischen `src/i18n/locales` und `public/locales` mittels `scripts/sync_locales.sh`.
5.  **Harter Rebuild**: Ausführung von `docker-compose up --build -d frontend`, um sicherzustellen, dass die neuen Übersetzungen und Komponenten fest in das Produktions-JS-Bundle (Vite) kompiliert werden und keine Browser-Cache-Reste überleben.
...
- **Redundante JSON-Dateien**: Änderungen in `src` kamen nicht im Webserver an, da dieser `public` bevorzugt. Lösung: Strikte Synchronisation mittels `scripts/sync_locales.sh`.
...
## 📊 ERGEBNIS
Das Frontend enthält keine hardcodierten englischen Status-Begriffe mehr. Das Dashboard für den Directeur Général (DG) ist nun vollständig lokalisiert und reagiert korrekt auf den Sprachwechsel. Die Konsistenz der Übersetzungsdaten ist durch das `sync_locales.sh` Skript und die Dokumentation in `CONTEXT_FOR_AI.md` dauerhaft gesichert.
